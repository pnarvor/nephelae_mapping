...
def run_gp_circular(mf, save_path=""):
    ...
    sigma_f = np.append(sigma_f, 2)  # lwc 4 # constant_value parameter is variance
    ...
    if (mf.size != 0):
        ...
        plt.figure()
        plt.title("Prior Mean on Vertical Wind Speed")
        plt.xlabel("x coordinate(km)")
        plt.ylabel("y coordinate(km)")
    ...
    plt.text(num_points[-1], sigma_f[-1], round(sigma_f[-1], 2), transform=plt.gca().transData, fontsize=16,
             ha='center',
    ...
    plt.text(num_points[0], mse[0], round(mse[0], 2), transform=plt.gca().transData, fontsize=16, ha='center',
             va='bottom',
...
def run_gp_criss_cross(mf, save_path=""):
    ...
    if (mf.size != 0):
        ...
        if (save_path):
            plt.savefig(save_path + 'prior_mean.png')
#  if(traj == "circular"):
# Uncomment for loop below for circulr trajectory
#   for r in  (34, 29, 24, 19, 14, 9): #circ traj
#       x, ytt = circular(Xt_grid, y, sigma_n, r) #TODO: Mean prior with circular!!
